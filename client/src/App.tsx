...
import { Sparkles, LogOut, ShieldAlert, Plus } from 'lucide-react'
  ...
  return (
    <Router>
      <div className="min-h-screen bg-[#050811] text-slate-100 flex flex-col md:flex-row antialiased selection:bg-violet-500 selection:text-white">
        ...
        {/* Mobile Header Bar */}
        <header className="md:hidden sticky top-0 z-30 bg-[#0C1222]/90 backdrop-blur-2xl border-b border-white/[0.08] px-3.5 py-2.5 flex items-center justify-between shadow-lg shadow-black/50">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              onClick={() => { sounds.playTap(); setIsQuickActionOpen(true); }}
              className="p-1.5 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white font-bold text-xs flex items-center gap-1 shadow-md shadow-violet-600/30 active:scale-95 transition"
            >
              <Plus className="w-3.5 h-3.5" />
              <span className="text-[10px]">Tạo nhanh</span>
            </button>
