...
export const TasksBoard: React.FC = () => {
  ...
  const eisenhowerQuadrants = [
    ...
    },
    {
      id: 'schedule',
      title: 'Q2: Schedule',
      desc: '📅 Quan trọng • Lên lịch dài hạn',
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-300 select-none pb-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="text-[10px] font-bold text-violet-400 uppercase tracking-widest flex items-center gap-1">
            ...
            <span>Năng Suất Vượt Trội</span>
          </div>
          <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight">Nhiệm Vụ & Ma Trận</h1>
        </div>
      ...
      <div className="p-1 glass-card rounded-2xl flex items-center gap-1 border border-white/[0.08]">
        <button
      ...
      {activeTab === 'eisenhower' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
          {eisenhowerQuadrants.map(quad => (
            <div
              key={quad.id}
              className={`glass-card rounded-3xl p-3.5 sm:p-4 border ${quad.color} shadow-lg space-y-2.5`}
            >
                ...
                {quad.tasks.length === 0 ? (
                  ...
                  quad.tasks.map(task => {
                    ...
                    return (
                      <div
                        key={task.id}
                        className={`p-2.5 rounded-2xl bg-slate-900/80 border transition-all flex items-center justify-between gap-2.5 ${
                      ...
                      >
                        ...
                        {!isDone && (
                          <button
                            ...
                            title="Tập trung"
                            className="p-1 rounded-lg bg-rose-500/10 text-rose-300 hover:bg-rose-600 hover:text-white transition shrink-0"
                          >
        ...
        <div className="space-y-2">
          {tasks.map(task => {
            ...
            return (
              <div
                key={task.id}
                className="glass-card rounded-2xl p-3 border border-white/[0.08] flex items-center justify-between gap-3"
              >
                <button
                  onClick={() => toggleTaskStatus(task.id)}
