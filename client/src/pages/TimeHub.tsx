...
import { TimeLedgerView } from './TimeLedgerView'
...
export const TimeHub: React.FC<Props> = ({ onOpenFullscreenFocus }) => {
  ...
  // Read tab parameter from URL query param (?tab=schedule | tracking | ledger) or pathname
  const queryTab = searchParams.get('tab')
  const defaultTab = location.pathname.startsWith('/tracking')
    ? 'tracking'
    : location.pathname.startsWith('/ledger')
    ? 'ledger'
    : 'schedule'
  const activeTab: 'schedule' | 'tracking' | 'ledger' =
    queryTab === 'tracking' || queryTab === 'ledger' || queryTab === 'schedule'
      ? queryTab
      : defaultTab

  const handleTabChange = (newTab: 'schedule' | 'tracking' | 'ledger') => {
    sounds.playTap()
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-hidden bg-[#F8FAFC]">
      {/* ── Main Content Container (Schedule vs Live Tracking vs Time Ledger) ── */}
